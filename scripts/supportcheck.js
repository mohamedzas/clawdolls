...
(function() {
    ...
    if (missingFeatures.length === 0 && !isKasperskyScriptInjected) window["C3_IsSupported"] = true;
        ...
        else if (/iphone|ipad|ipod/i.test(ua)) msgText += "<br><br>Note: the <strong>iOS simulator</strong> is not currently supported due to an <a href='https://bugs.webkit.org/show_bug.cgi?id=191064'>Apple bug</a>. If you are using the simulator, try testing on a real device instead.";
        else if ((/msie/i.test(ua) || /trident/i.test(ua)) && !/edge\//i.test(ua)) msgText += "<br><br>Note: <strong>Internet Explorer</strong> is not supported. Try using <a href='https://www.google.com/chrome'>Chrome</a> or <a href='https://www.mozilla.org/firefox'>Firefox</a> instead.";
